...
export class ArticleService {
  ...
  getForEdit(id: number) {
    return this.http.get<Article>(`${this.apiUrl}/articles/${id}/edit`);
  }
  ...
  getMyArticles(page = 0, size = 100) {
    const params = new HttpParams().set('page', page).set('size', size);
    return this.http.get<Page<Article>>(`${this.apiUrl}/articles/my`, { params }).pipe(
      map(p => p.content)
    );
  }
}
